...
DEFAULT_OPTIONS = {
    ...
    "behaviors": [
        "集中して取り組まれていました",
        "質問しながら丁寧に進めていらっしゃいました",
        "試行錯誤しながら進めていらっしゃいました",
        "落ち着いた様子で取り組まれていました",
        "リサーチしながらイメージを膨らませていらっしゃいました",
        "操作に少し苦戦されていました",
        "スムーズに進めることができていました",
        "楽しそうに取り組まれていました",
        "1人で集中してプレイされていました",
    ...
    "impressions": [
        ...
        "緊張されているご様子でした",
        "表情が硬い印象でした",
        "積極的にコミュニケーションをとっていらっしゃいました",
        "他の利用者に声をかけるなど思いやりが見られました",
        "自分のペースで無理なく取り組まれていました",
        "体調が良さそうなご様子でした",
        "少し疲れているご様子でした",
        "意欲的に新しいことに挑戦されていました",
        "前回より成長が見られました",
...
## 文体ルール（最重要）
- 「〜である」「〜であった」は絶対に使わないこと
- 「〜されていました」「〜でした」「〜していらっしゃいました」のような丁寧で柔らかい文体にする
- 「〜に取り組まれていました」「〜されていました」を基本にする
- 末尾に「電話対応 11:00、14:00」のような記載がある場合、これは利用者ではなく職員が行った業務です。「〜されていました」等の利用者主語の表現にせず、必ず「電話対応は〇時に行いました。」と職員主語・「行いました」の表現で書き換えてください（例: 「電話対応 11:00、14:00」→「電話対応は11時、14時に行いました。」）
...
× 実施し → ○ 実施され
× 予定である → ○ 予定です
× 取り組む予定である → ○ 取り組んでいく予定です

## 敬語ルール
- 「てらっしゃいました」→「ていらっしゃいました」（「い」を省略しない）
- 敬語の脱落や誤りがあれば必ず修正する

## 主語・表現ルール
- 利用者の記録なので、利用者が主語になる表現にする
- 「〜が行われていました」「〜が実施されていました」のように活動が主語になる表現は避ける
  × 「eスポーツ実習が行われていました」→ ○ 「eスポーツ実習に取り組まれていました」
  × 「動画編集を実施されていました」→ ○ 「動画編集に取り組まれていました」
- 文末表現が3回以上連続で同じにならないよう変化をつける
  （「〜されていました」「〜ご様子でした」「〜していらっしゃいました」「〜見られました」などを使い分ける）
...
# クラウド版（Render）では環境変数でプロンプトを永続化
# Renderの無料プランはデプロイ時にファイルシステムがリセットされるため
# ファイル保存だけではプロンプトが消えてしまう
_runtime_prompt = os.environ.get("CUSTOM_CHECK_PROMPT", "")


def load_check_prompt():
    # 1. ランタイムメモリ（管理画面から保存した場合）
    if _runtime_prompt:
        return _runtime_prompt
    # 2. ファイル（EXE版で有効）
    saved = _load_json(PROMPT_FILE, {}).get("prompt", "")
    if saved:
        return saved
    # 3. デフォルト
    return DEFAULT_CHECK_PROMPT


def save_check_prompt(prompt):
    global _runtime_prompt
    _runtime_prompt = prompt
    # ファイルにも保存（EXE版用 + クラウド版でも再起動まで有効）
    _save_json(PROMPT_FILE, {"prompt": prompt})


def _migrate_options(options):
    """古い文体（〜ていた）を新しい文体（〜ていました）に自動変換"""
    replacements = {
        "集中して取り組んでいた": "集中して取り組まれていました",
        "質問しながら丁寧に進めていた": "質問しながら丁寧に進めていらっしゃいました",
        "試行錯誤しながら進めていた": "試行錯誤しながら進めていらっしゃいました",
        "落ち着いた様子で取り組んでいた": "落ち着いた様子で取り組まれていました",
        "リサーチしながらイメージを膨らませていた": "リサーチしながらイメージを膨らませていらっしゃいました",
        "操作に少し苦戦していた": "操作に少し苦戦されていました",
        "スムーズに進めることができていた": "スムーズに進めることができていました",
        "楽しそうに取り組んでいた": "楽しそうに取り組まれていました",
        "1人で集中してプレイしていた": "1人で集中してプレイされていました",
        "他の利用者と協力していた": "他の利用者と協力されていました",
        "落ち着いた様子だった": "落ち着いたご様子でした",
        "緊張している様子だった": "緊張されているご様子でした",
        "表情が硬い印象だった": "表情が硬い印象でした",
        "積極的にコミュニケーションをとっていた": "積極的にコミュニケーションをとっていらっしゃいました",
        "他の利用者に声をかけるなど思いやりが見られた": "他の利用者に声をかけるなど思いやりが見られました",
        "自分のペースで無理なく取り組めていた": "自分のペースで無理なく取り組まれていました",
        "体調が良さそうだった": "体調が良さそうなご様子でした",
        "少し疲れている様子だった": "少し疲れているご様子でした",
        "意欲的に新しいことに挑戦していた": "意欲的に新しいことに挑戦されていました",
        "前回より成長が見られた": "前回より成長が見られました",
    }
    for key in ("behaviors", "impressions"):
        if key in options:
            options[key] = [replacements.get(item, item) for item in options[key]]
    return options
...
def load_options():
    ...
    # 古い文体を自動変換
    options = _migrate_options(options)
    return options
...
ACTIVITY_OPENERS_AM = [
    "午前は{activity}に取り組まれていました。",
    "午前中は{activity}に取り組まれていました。",
    "午前は{activity}を実施されていました。",
]

ACTIVITY_OPENERS_PM = [
    "午後からは{activity}に取り組まれていました。",
    "午後は{activity}に取り組まれていました。",
    "午後からは{activity}を実施されていました。",
]
    ...
    if am_activities:
        ...
        if am_next:
            ...
            parts.append(random.choice([
                f"次回以降は{action_text}予定です。",
                f"今後は{action_text}方針です。",
            ]))
